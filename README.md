...
subb <- smooth(sub, method = "ksmooth",smoothness = 10)
 ggg<-leaflet()%>%
    addPolylines(data=mytrip,fillOpacity=0,color="black",stroke=T,weight = 10)%>%
    addPolylines(data=mytrip,fillOpacity=0,color="white",stroke=T,weight = 2)%>%
    addMarkers(data=xy)%>%addTiles()%>%
    addRasterImage(pred2, colors = "blue", opacity = 0.8)%>%
    addPolygons(data=subb,fillOpacity=0.5,color="red",stroke=T)%>%
